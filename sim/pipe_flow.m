...
function [dP,P_fan] = deltaP(cfm,fanP,fanQ,k_filter,N_cells,p)
    ...
    areas = pi/4 * diams.^2; % m^2
    vels = cfm / (.5886 * 3600) ./ areas; % m/s
    vels(2) = vels(2) / N_cells; % heater flow splits, this assumes no bypass and that multiple heaters are in parallel
    Re = p.rho * vels .* diams / p.mu; % Reynolds
    
    if Re < 4000
        f = 64 ./ Re; % laminar
    else
        f = (-1.8 * log10(6.9 ./ Re + (eps./diams/3.7).^1.11)).^-2; % turbulent, using Colebrook-White
    end
